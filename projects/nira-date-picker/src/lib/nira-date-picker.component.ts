...
// import { ConfigStorageService, ConsoleService } from 'nira-lib/Services';
    ...
    >
      ...
      >
        ...
        <input
          ...
          matInput
          [ngModel]="result.replaceAll('-', '/') | persianDigits"
    ...
    <ng-template class="dialog" #dpDialog>
      <div class="dp-header">
        ...
        <label style="color: gray; font-weight: 500; font-size: 20px">
          {{ months[month] | persianDigits }}</label
        >
        <button
          class="month-btn"
          mat-button
        ...
        <div *ngIf="calendarType == 'en'" class="Grid Grid--wrap day-container">
          ...
          >
            <ng-container *ngIf="day !== 0">
              {{ day | persianDigits }}</ng-container
            >
            <br />
            <div>
              <label class="best-fare-label" style="font-size: 9px">
                {{ this.findBestFare(year, month, day) | persianDigits }}
              </label>
            </div>
          </span>
        </div>
      ...
      <div mat-dialog-content style="padding: 0px">
        ...
        >
          ...
          >
            <br />
            <div>
              <label style="font-size: 14px; padding: 2px"
                >{{ decade | persianDigits }} -
                {{ decade + 10 | persianDigits }}</label
              >
            </div>
          </span>
        </div>
...
export class NiraDatePickerComponent implements OnChanges, OnInit, DoCheck {
  ...
  public updateMonth(e?: Event, type?: string) {
    ...
    if (this.calendarType == 'en') {
      ...
      let Jdate = moment.default(Jstr, 'jYYYY/jM/jD'); // Parse a Jalaali date
  ...
  public setDefaultValues() {
    ...
    if (strdefaultDate != undefined && strdefaultDate.length > 8) {
      ...
      if (+year > 1500) {
        ...
        this.m = moment.default(miladiStr);
