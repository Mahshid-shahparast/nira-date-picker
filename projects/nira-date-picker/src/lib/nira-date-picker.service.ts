...
export class NiraDatePickerService {
  ...
  public getShamsiDateStr(): string {
    ...
    let mo = moment.default(miladi, 'YYYY/MM/DD');
    let y = mo.jYear();
    let m = mo.jMonth() + 1;
    let d = mo.jDate();
  ...
  public incYear(n: number) {
    var d = moment.default(this.getMiladiDateTimeStr());
    var dt = d.add(n, 'years');
    this.setMiladyYDM(dt.year(), dt.month() + 1, dt.date());
  }

  public incMonth(n: number) {
    var d = moment.default(this.getMiladiDateStr());
  ...
  public incHours(n: number) {
    let d = moment.default(this.getMiladiDateTimeStr());
